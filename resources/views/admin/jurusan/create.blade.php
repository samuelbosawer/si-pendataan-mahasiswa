...
@section('title')
{{$judul ?? 'Tambah Data Jurusan' }} - Admin
@endsection
@section('content')
    <!--  BEGIN CONTENT AREA  -->
                        ...
                        <div class="statbox widget box box-shadow">
                            ...
                            <div class="widget-content widget-content-area">
                                ...
                                @if (Request::segment(4) == 'ubah')
                                    <form action="{{ route('dashboard.jurusan.update', $data->id) }}" method="post"
                                        enctype="multipart/form-data" class="row g-3">
                                        @method('PUT')
                                    @else
                                        <form action="{{ route('dashboard.jurusan.store') }}" method="post"
                                            enctype="multipart/form-data" class="row g-3">
                                @endif
                                @csrf
                                <div class="col-md-12">
                                    <label for="nama_jurusan" class="form-label">Nama Jurusan <span class="text-danger" >*</span> </label>
                                    <input type="text" name="nama_jurusan" @if (Request::segment(3) == 'detail') disabled @endif  value="{{old('nama_jurusan') ?? ($data->nama_jurusan ?? '')}}" class="form-control" id="nama_jurusan">
                                    @if ($errors->has('nama_jurusan'))
                                        <label class="text-danger"> {{ $errors->first('nama_jurusan') }}
                                        </label>
                                    @endif
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <a href="{{route('dashboard.jurusan.ubah',$data->id)}}" class="btn btn-primary"> Ubah Data</a>
                                    <a href="{{route('dashboard.jurusan')}}" class="btn btn-success"> Kembali</a>
                                    @else
